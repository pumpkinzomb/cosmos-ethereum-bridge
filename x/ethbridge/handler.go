...
func NewHandler(oracleKeeper oracle.Keeper, bankKeeper bankkeeper.Keeper, cdc *codec.LegacyAmino, codespace string) sdk.Handler {
	...
	msgServer := keeper.NewMsgServerImpl(oracleKeeper)

	return func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		ctx = ctx.WithEventManager(sdk.NewEventManager())
		
		switch msg := msg.(type) {
		case *types.MsgMakeEthBridgeClaim:
			// return handleMsgMakeEthBridgeClaim(ctx, cdc, oracleKeeper, bankKeeper, msg, codespace)
			return nil, nil
		default:
			return nil, sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "Unrecognized ethbridge message type: %v", msg.Type())
		}
	}
}
